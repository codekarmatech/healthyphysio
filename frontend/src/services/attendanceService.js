...
class AttendanceService extends BaseService {
  ...
  async requestAttendanceChange(attendanceId, requestedStatus, reason) {
    ...
    try {
      ...
      const response = await api.post(`${this.basePath}${attendanceId}/request-change/`, data);
  ...
  async getLeaveApplications(therapistId, params = {}) {
    try {
      // Try to call the real API endpoint
      return await api.get(`${this.basePath}leave/therapist/${therapistId}/`, { params });
    } catch (error) {
      // If the API endpoint returns 404, it means the feature is not yet implemented
      if (error.response && error.response.status === 404) {
  ...
  async getPatientTherapistAttendance(therapistId, patientId, year, month) {
    try {
      // Try to call the real API endpoint
      const response = await api.get(`${this.basePath}therapist/${therapistId}/patient/${patientId}/?year=${year}&month=${month}`);
      return response;
    } catch (error) {
      // If the API call fails, use mock data
      console.warn('Error fetching patient-therapist attendance, using mock data:', error.message);
