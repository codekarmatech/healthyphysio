...
class DashboardSummaryTests(APITestCase):
    ...
    def test_therapist_dashboard_summary(self):
        # Test therapist viewing their own dashboard
        self.client.force_authenticate(user=self.therapist_user)
        url = reverse('therapist-dashboard-summary')
        response = self.client.get(url)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('upcoming_appointments_count', response.data)
        self.assertIn('today_appointments_count', response.data)
        self.assertIn('total_patients', response.data)
        self.assertIn('pending_assessments_count', response.data)
        self.assertIn('monthly_earnings', response.data)
        self.assertIn('equipment_allocations_count', response.data)
        self.assertIn('equipment_requests_count', response.data)
        self.assertIn('active_visits_count', response.data)
        self.assertIn('pending_reports_count', response.data)
        self.assertIn('pending_treatment_plan_change_requests_count', response.data)
        self.assertIn('recent_appointments', response.data)
        self.assertIn('treatment_plan_change_requests', response.data)

    def test_admin_viewing_therapist_dashboard_summary(self):
        # Test admin viewing a specific therapist's dashboard
        self.client.force_authenticate(user=self.admin_user)

        # First, get the therapist ID
        therapist = Therapist.objects.get(user=self.therapist_user)

        url = reverse('therapist-dashboard-summary') + f'?therapist_id={therapist.id}'
        response = self.client.get(url)
